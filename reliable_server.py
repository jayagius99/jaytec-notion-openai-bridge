...
def _retryable_single_attempt_dispatch(underlying, *, model: str):
    def single_attempt(packet):
        try:
            ...
            return {"status": "TIMEOUT", "model": model, "findings": [], "evidence": [], "unresolved_items": ["worker_timeout"]}
        except RateLimitError as exc:
            retry_after = getattr(exc, "retry_after", None)
            suffix = f":retry_after={retry_after}" if retry_after is not None else ""
            return {"status": "RATE_LIMITED", "model": model, "findings": [], "evidence": [], "unresolved_items": [f"retryable_rate_limit{suffix}"]}
        except ProviderUnavailableError as exc:
            retry_after = getattr(exc, "retry_after", None)
            suffix = f":retry_after={retry_after}" if retry_after is not None else ""
            return {"status": "RATE_LIMITED", "model": model, "findings": [], "evidence": [], "unresolved_items": [f"retryable_provider_unavailable{suffix}"]}
        except CircuitOpenError:
            return {"status": "RATE_LIMITED", "model": model, "findings": [], "evidence": [], "unresolved_items": ["retryable_provider_circuit_open"]}

    return single_attempt


def _transient_safe_execute_task_packet_json(packet_json: str, *, registry: ExecutionRegistry, idempotency_store: str, codex_dispatch: Any, gemini_dispatch: Any) -> str:
    return _ORIGINAL_EXECUTE(packet_json, registry=TransientAwareRegistry(registry), idempotency_store=idempotency_store, codex_dispatch=codex_dispatch, gemini_dispatch=gemini_dispatch)


def _bounded_legacy_codex_dispatch(*, openai_client, codex_model, circuit):
    underlying = _ORIGINAL_BUILD_CODEX(openai_client=openai_client, codex_model=codex_model, circuit=circuit, codex_timeout_s=max(1.0, LEGACY_SYNC_PROVIDER_TIMEOUT_S))
...
def _bounded_legacy_gemini_dispatch(*, openrouter_client, gemini_model, gemini_timeout_s, circuit):
    underlying = _ORIGINAL_BUILD_GEMINI(openrouter_client=openrouter_client, gemini_model=gemini_model, gemini_timeout_s=max(1.0, min(float(gemini_timeout_s), LEGACY_SYNC_PROVIDER_TIMEOUT_S)), circuit=circuit)
...
class ReliableDurableWorkerPool:
    def __init__(self, queue: ReliableDurableTaskQueue, execute_packet, *, instance_id: str, worker_count: int, poll_seconds: float, lease_seconds: int):
        count = max(1, int(worker_count))
        self.workers = [
            ReliableDurableTaskWorker(queue, execute_packet, owner=f"render:{instance_id}:worker:{index}", execution_room_id=f"durable-worker:{instance_id}:{index}", poll_seconds=poll_seconds, lease_seconds=lease_seconds)
...
class GuardianBackgroundLoop:
    def __init__(self, guardian: ReliabilityGuardian, queue: ReliableDurableTaskQueue, *, interval_seconds: float):
    ...
    def _loop(self) -> None:
        while not self._stop.is_set():
            try:
                ...
                try:
                    self.queue.record_incident("GUARDIAN_LOOP_EXCEPTION", detail={"error_class": type(exc).__name__, "message": str(exc)[:1000]})
                except Exception:
                    pass
    ...
    def start(self) -> None:
        if self._thread and self._thread.is_alive():
            return
        self._thread = threading.Thread(target=self._loop, name="jaytec-guardian-lite-loop", daemon=True)
        self._thread.start()
...
def create_mcp_app():
    ...
    if legacy_server.DATABASE_URL:
        queue = ReliableDurableTaskQueue(legacy_server.DATABASE_URL, max_parallel=MAX_PARALLEL_DURABLE_JOBS)
        ...
        durable_registry = PostgresExecutionRegistry(database_url=legacy_server.DATABASE_URL, ttl_seconds=ExecutionRegistry().ttl_seconds)
        durable_registry.ensure_schema()

        durable_openai = OpenAI(api_key=legacy_server.OPENAI_API_KEY)
        durable_openrouter = OpenAI(api_key=legacy_server.OPENROUTER_API_KEY, base_url=legacy_server.OPENROUTER_BASE_URL) if legacy_server.OPENROUTER_API_KEY else None
        durable_codex_circuit = CircuitBreaker(failure_threshold=legacy_server.CIRCUIT_FAILURE_THRESHOLD, reset_after_seconds=legacy_server.CIRCUIT_RESET_SECONDS)
        durable_gemini_circuit = CircuitBreaker(failure_threshold=legacy_server.CIRCUIT_FAILURE_THRESHOLD, reset_after_seconds=legacy_server.CIRCUIT_RESET_SECONDS)
        durable_codex_dispatch = _retryable_single_attempt_dispatch(build_codex_dispatch(openai_client=durable_openai, codex_model=legacy_server.CODEX_MODEL, circuit=durable_codex_circuit, codex_timeout_s=DURABLE_CODEX_TIMEOUT_S), model=EXPECTED_CODEX_MODEL)
        if durable_openrouter is not None:
            durable_gemini_dispatch = _retryable_single_attempt_dispatch(build_gemini_dispatch(openrouter_client=durable_openrouter, gemini_model=legacy_server.GEMINI_MODEL, gemini_timeout_s=DURABLE_GEMINI_TIMEOUT_S, circuit=durable_gemini_circuit), model=EXPECTED_GEMINI_MODEL)
        else:
            durable_gemini_dispatch = durable_gemini_circuit.guard(lambda _packet: (_ for _ in ()).throw(RuntimeError("OPENROUTER_API_KEY is not configured on this bridge")))

        def _execute_durable(packet_json: str) -> Mapping[str, Any]:
            text = _transient_safe_execute_task_packet_json(packet_json, registry=durable_registry, idempotency_store="postgres", codex_dispatch=durable_codex_dispatch, gemini_dispatch=durable_gemini_dispatch)
        ...
        worker = ReliableDurableWorkerPool(queue, _execute_durable, instance_id=instance_id, worker_count=MAX_PARALLEL_DURABLE_JOBS, poll_seconds=DURABLE_WORKER_POLL_S, lease_seconds=DURABLE_WORKER_LEASE_S)
        guardian_loop = GuardianBackgroundLoop(guardian, queue, interval_seconds=GUARDIAN_INTERVAL_S)
    ...
    @mcp.tool
    def submit_task_packet_durable(packet_json: str, source_shared_state_version: int = 0, priority: int = 100, execution_room_id: str = "") -> str:
        ...
        try:
            snapshot = queue.submit(packet_json, source_shared_state_version=source_shared_state_version, priority=priority, source_execution_room_id=execution_room_id or None)
            return _json({"available": True, "accepted": True, "preferred_poll_tool": "task_packet_status", "snapshot": snapshot})
        except (PacketValidationError, ValueError, RuntimeError) as exc:
            return _json({"available": True, "accepted": False, "error_class": type(exc).__name__, "error": str(exc)})
    ...
    def task_packet_status(job_id: str = "", idempotency_key: str = "") -> str:
        ...
        if queue is None:
            return _json({"available": False, "reason": "DATABASE_URL_NOT_CONFIGURED"})
        try:
            return _json({"available": True, "snapshot": queue.status(job_id=job_id or None, idempotency_key=idempotency_key or None)})
        except Exception as exc:
            return _json({"available": True, "error_class": type(exc).__name__, "error": str(exc)})

    @mcp.tool
    def reliability_status() -> str:
        """Return durable worker/Guardian health and current operational counts."""
        if queue is None:
            return _json({"runtime_id": RUNTIME_ID, "database_available": False, "durable_worker_enabled": False, "guardian_loop_enabled": False})
        return _json({
    ...
    @mcp.tool
    def record_reliability_incident(event_type: str, detail_json: str = "{}", job_id: str = "") -> str:
        ...
        try:
            ...
            event = queue.record_incident(event_type, job_id=job_id or None, detail=detail)
    ...
    mcp._jaytec_reliability = {"queue": queue, "worker": worker, "guardian": guardian, "guardian_loop": guardian_loop, "workload_read_model": workload_read_model}
    return mcp


def main() -> None:
    mcp = create_mcp_app()
    mcp.run(transport="http", host="0.0.0.0", port=legacy_server.PORT, stateless_http=True, host_origin_protection=False)
