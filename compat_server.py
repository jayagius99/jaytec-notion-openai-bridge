...
def _compat_target(task: str) -> Optional[Tuple[str, Dict[str, Any]]]:
    ...
    if task.startswith(RUN_GUARDIAN_PREFIX):
        ...
        return "run_guardian_lite", {"auto_repair": _strict_bool(body.get("auto_repair"), field="auto_repair", default=False)}
    ...
    if task.startswith(DURABLE_SUBMIT_PREFIX):
        ...
        return "submit_task_packet_durable", {
            ...
            "source_shared_state_version": _strict_int(body.get("source_shared_state_version"), field="source_shared_state_version", default=0),
            "priority": _strict_int(body.get("priority"), field="priority", default=100),
            "execution_room_id": _strict_string(body.get("execution_room_id"), field="execution_room_id", default="").strip(),
        }
    ...
    if task.startswith(DURABLE_STATUS_PREFIX):
        ...
        idempotency_key = _strict_string(body.get("idempotency_key"), field="idempotency_key", default="").strip()
        if not job_id and not idempotency_key:
            raise ValueError("job_id or idempotency_key is required")
        return "task_packet_status", {"job_id": job_id, "idempotency_key": idempotency_key}
    ...
    if task.startswith(RECORD_INCIDENT_PREFIX):
        ...
        event_type = _strict_string(body.get("event_type"), field="event_type", default="").strip()
        if not event_type:
        ...
        return "record_reliability_incident", {"event_type": event_type, "detail_json": _json(dict(detail)), "job_id": job_id}
...
def rewrite_jsonrpc_payload(payload: Any) -> Any:
    ...
    if isinstance(payload, list):
        return [_rewrite_call(item) if isinstance(item, dict) else copy.deepcopy(item) for item in payload]
    return payload
...
class LegacyCatalogCompatMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope.get("type") != "http" or scope.get("method") != "POST" or scope.get("path") not in MCP_COMPAT_PATHS or not _is_json_content_type(scope):
    ...
    async def _reject_oversized(self, send):
        ...
        await send({"type": "http.response.start", "status": 413, "headers": [(b"content-type", b"text/plain; charset=utf-8"), (b"content-length", str(len(body)).encode("ascii"))]})
...
def create_http_app(mcp=None):
    ...
    return server.http_app(middleware=[Middleware(LegacyCatalogCompatMiddleware)], stateless_http=True, host_origin_protection=False)


def main() -> None:
    uvicorn.run(create_http_app(), host="0.0.0.0", port=legacy_server.PORT, log_level="info")
